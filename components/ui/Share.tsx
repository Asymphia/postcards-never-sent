...
const Share = ({ postcardId }: { postcardId: number }) => {
    ...
    return (
        <button className="cursor-pointer relative" type="button" onClick={handleCopy}>
            <svg className="size-6 fill-accent transition-all hover:fill-accent-dark" viewBox="0 0 800 800" xmlns="http://www.w3.org/2000/svg">
            ...
            <span className={`absolute top-0  px-2 py-1 bg-accent text-white text-xs rounded-sm transition-all ${copied ? "-left-[70px] opacity-100" : "-left-[30px] opacity-0"}`}>
                Copied!
            </span>
        </button>
    )
