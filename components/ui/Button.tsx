...
const Button = ({ text, type="primary", onClick }: { text: string, type?: "primary" | "secondary", onClick?: () => void }) => {
    ...
    return (
        <button disabled={ status.pending } className={`xl:min-w-52 xl:px-20 md:px-12 px-8 py-2 rounded-sm cursor-pointer transition-all ${buttonStyle[type]} ${ status.pending && "opacity-90" }`} onClick={onClick}>
            {
                status.pending ? "Sending..." : text
            }
        </button>
    )
}
